...
const Header = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<header class="${"fixed top-0 left-0 right-0 z-50 h-[88px] transition-all duration-400 px-6 md:px-10 flex items-center justify-between " + escape(
    ...
    true
  )}"><i class="ri-dashboard-line text-lg"></i> <span class="hidden xl:inline" data-svelte-h="svelte-czh1o1">Admin Portal</span></a>` : `${user.role === "agent" ? `<a href="/agent" class="${"flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all " + escape(
    "text-background-50 hover:text-accent-450",
    true
  )}"><i class="ri-user-line text-lg"></i> <span class="hidden xl:inline" data-svelte-h="svelte-l2mwmj">Agent Portal</span></a>` : `<div class="${"flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all " + escape(
    "text-background-100/90",
    true
  )}"><i class="ri-user-smile-line text-lg"></i> <span class="hidden xl:inline">Hi, ${escape(user.first_name)}</span></div>`}`} <form method="POST" action="/login?/logout" class="inline-flex"><button type="submit" class="${"flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all " + escape(

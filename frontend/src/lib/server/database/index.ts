...
// Hash function helper for seeding (matches the one in auth.ts)
...
// Database file path inside src/lib/server/database
...
if (userCount.count === 0) {
  ...
  // Create Agent Profile for Agent
  ...
  insertProperty.run(
    prop1Id, agentId, 'Luxury 4 Bedroom Villa in Nyambadwe',
    ...
    'house', 'sale', 85000000, 'MK', 4, 3, 2, 280,
    'Soche View Crescent, Nyambadwe', 'Blantyre', 'Southern Region', 'Malawi', -15.776, 35.018,
    'approved', 1, now, now
  );

  insertProperty.run(
    prop2Id, agentId, 'Modern 2 Bedroom Apartment in Limbe',
    ...
    'apartment', 'rent', 350000, 'MK', 2, 2, 1, 110,
    'Kanjedza Drive, Limbe', 'Blantyre', 'Southern Region', 'Malawi', -15.808, 35.056,
    'approved', 1, now, now
  );

  insertProperty.run(
    prop3Id, agentId, 'Prime Commercial Land in Lilongwe Area 43',
    ...
    'land', 'sale', 120000000, 'MK', null, null, null, 6000,
    'Off Presidential Highway, Area 43', 'Lilongwe', 'Central Region', 'Malawi', -13.931, 33.812,
    'approved', 0, now, now
  );
  ...
  // Prop 1 has Swimming Pool, Borehole, Security
  ...
  // Prop 2 has Borehole, Solar Power, Security, WiFi

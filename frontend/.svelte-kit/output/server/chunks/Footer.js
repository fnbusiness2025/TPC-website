import { c as create_ssr_component, s as subscribe, e as escape, v as validate_component, f as each, d as add_attribute } from "./ssr.js";
...
const Header = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  })}</nav> <div class="hidden lg:flex items-center gap-6 pl-6 border-l border-background-200/20">${user ? ` ${user.role === "admin" ? `<a href="/admin" class="${"flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all " + escape(
    "text-background-50 hover:text-accent-450",
    true
  )}"><i class="ri-dashboard-line text-lg"></i> <span class="hidden xl:inline">${escape(user.first_name)} ${escape(user.last_name)}</span></a>` : `${user.role === "agent" ? `<a href="/agent" class="${"flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all " + escape(
    "text-background-50 hover:text-accent-450",
    true
  )}"><i class="ri-user-line text-lg"></i> <span class="hidden xl:inline">${escape(user.first_name)} ${escape(user.last_name)}</span></a>` : `<div class="${"flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all " + escape(
    "text-background-100/90",
    true
  )}"><i class="ri-user-smile-line text-lg"></i> <span class="hidden xl:inline">Hi, ${escape(user.first_name)} ${escape(user.last_name)}</span></div>`}`} <form method="POST" action="/login?/logout" class="inline-flex"><button type="submit" class="${"flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all " + escape(
    "text-red-300 hover:text-red-400",
    true
  )}"><i class="ri-logout-box-r-line text-lg"></i> <span class="hidden xl:inline" data-svelte-h="svelte-1iq4a37">Sign Out</span></button></form>` : ` ${each(utilityNav, (item) => {

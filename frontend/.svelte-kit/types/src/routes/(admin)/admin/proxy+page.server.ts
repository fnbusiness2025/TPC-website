...
export const load = ({ cookies }: Parameters<PageServerLoad>[0]) => {
  ...
  try {
    // 1. Fetch counts
    const totalProp = db.prepare('SELECT COUNT(*) as count FROM properties').get() as { count: number };
    const pendingPropCount = db.prepare("SELECT COUNT(*) as count FROM properties WHERE status = 'pending'").get() as { count: number };
    const pendingAgentCount = db.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'agent' AND status = 'pending'").get() as { count: number };
    const totalInquiries = db.prepare('SELECT COUNT(*) as count FROM property_inquiries').get() as { count: number };
...
export const actions = {
  approveProperty: async ({ request, cookies }: import('./$types').RequestEvent) => {
    const user = authenticateUser(cookies);
  ...
  rejectProperty: async ({ request, cookies }: import('./$types').RequestEvent) => {
    const user = authenticateUser(cookies);
    if (!user || user.role !== 'admin') return fail(403, { error: true, message: 'Unauthorized' });
  ...
  approveAgent: async ({ request, cookies }: import('./$types').RequestEvent) => {
    const user = authenticateUser(cookies);
    if (!user || user.role !== 'admin') return fail(403, { error: true, message: 'Unauthorized' });
  ...
  rejectAgent: async ({ request, cookies }: import('./$types').RequestEvent) => {
    const user = authenticateUser(cookies);
    if (!user || user.role !== 'admin') return fail(403, { error: true, message: 'Unauthorized' });
